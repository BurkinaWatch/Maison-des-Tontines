...
export class MembershipsController {
    async inviteMember(req, res, next) {
        try {
            ...
            }).catch((error) => logger.warn("Invitation notification failed", { error: error.message }));
    ...
    async respondToInvitation(req, res, next) {
        try {
            ...
            if (accepted) {
                await tontineEngineModule.getCycleService().createCycles(membership.tontineId, membership.tontine.frequency, membership.tontine.startDate, await getPrisma().tontineMember.count({ where: { tontineId: membership.tontineId, status: "ACTIVE" } }));
            }
