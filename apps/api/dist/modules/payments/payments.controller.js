    ...
    async initiatePayout(req, res, next) {
        try {
            ...
            if (beneficiary) {
                ...
                }).catch((error) => logger.warn("Payout notification failed", { error: error.message }));
            }
            if (method === "WAVE") {
        ...
        try {
            ...
            if (success || terminalFailure) {
                await getPrisma().$transaction(async (tx) => {
                    ...
                    if (!current || ["SUCCESS", "FAILED", "CANCELLED", "EXPIRED"].includes(current.status))
                        return;
                ...
                if (transaction.userId) {
                    ...
                    }).catch((error) => logger.warn("Payment notification failed", { error: error.message }));
                }
            }
            res.json({
