...
export class WebhookController {
    ...
    async handleLiquidCashWebhook(req, res, next) {
        try {
            const provider = new LiquidCashProvider();
            const verification = await provider.verifyWebhook(req.body, String(req.headers["x-liquidcash-signature"] ?? ""));
    ...
    async confirmPayment(payload) {
        ...
        if (transaction.status === "SUCCESS" || transaction.contribution.status === "PAID")
            return;
