...
export function requireCycleInTontine(
  tontineParam = "tontineId",
  cycleParam = "cycleId",
) {
  return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    if (!req.userId) return res.status(401).json({ error: "Unauthorized" });
    ...
    try {
      ...
      if (!cycle) return res.status(404).json({ error: "Cycle not found in this tontine" });
      ...
      logger.error("Tontine cycle access check failed", { error: (error as Error).message });
      return res.status(500).json({ error: "Internal server error" });
    }
  };
}
