...
export class ContributionService {
  ...
  async markLate(contributionId: string, lateDays: number) {
    const prisma = getPrisma();
    ...
    const latePenaltyRateRule = tontineRules.find((r: any) => r.key === "latePenaltyRate");
    const penaltyRate = latePenaltyRateRule ? parseFloat(latePenaltyRateRule.value) : 0.05;

    const lateFee = await this.engine.calculateLatePenalty(
  ...
  async validateContribution(tontineId: string, cycleId: string, memberId: string, amount: number) {
    const prisma = getPrisma();
  ...
  private async updateTrustProfile(memberId: string, onTime: boolean, lateDays: number = 0) {
    const prisma = getPrisma();
